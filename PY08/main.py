...
def create_sandbox(body: SandBoxCreate, response: Response):
...
def get_sandbox(uuid: UUID, response: Response):
...
def patch_sandbox(if_match: Annotated[str | None, Header()], uuid: UUID, body: SandBoxCreate, response: Response):
...
def delete_sandbox(uuid: UUID, response: Response):
    if not uuid:
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {"detail": "no sandbox ID specified"}
    for sb in store["sandboxes"]:
...
def get_operations(id: UUID, response: Response):
    if not id:
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {"detail": "no sandbox ID specified"}
    response.status_code = status.HTTP_200_OK
